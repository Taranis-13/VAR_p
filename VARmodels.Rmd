...
```{r, test}
summary(ur.df(vardat[,"y1"],type="drift",lags=1))
summary(ur.df(vardat[,"y2"],type="drift",lags=1))
```

### recherche d'un décalage adapté.
```{r, recherche_lag}
lag=lags.select(vardat,lag.max =4,include="const")
print(lag)
```
...
library(vars)
varest= VAR(vardat,p = 2,type ="const",season = NULL,exogen = NULL)
print(varest)
```
