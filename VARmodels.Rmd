...
## Application sur 4 séries temporelles.

Nous tenterons d'appliquer le modèle VAR sur l'évolution des actions Total et Engie, du Cac_40 et de taux de change euro-dollar sur une année, en données journalières (septembre 2019 à septembre 2020). Du fait de l'épidémie surgie en début 2020 une rupture importante se produit début avril. L'étude commencera donc à cette période (2020-03-25).

```{r, load datas, echo=FALSE}
data=read.table(file="Action_Total.txt",sep="\t",header=T,dec=".")
h=5 # horizon de predict
l=dim(data)[1]-h 
deb=135  # début des données utilisées: 2020-03-25
data.train=data[deb:l,]
plot.ts(data.train[,-1],col="blue")

```
Les séries ne sont certainement pas stationnaires, ce que confirment les test de Dickey-Fuller.

```{r, Dickey, echo=FALSE,results='hide'}
for(i in c("Total","Engie","Cac_40" ,"Taux_Change")) 
{ print(i)
  print(summary(ur.df(data.train[,i],type="drift",lag=2)))
}
```
Nous allons différencier les variables ( $\Delta X_{t}=X_{t}- X_{t-1}$), et tenter un modèle VAR sur ces nouvelles variables ("D.Total","D.Engie","D.Cac_40" ,"D.Taux_Change") , après avoir vérifié leur stationnarité.

```{r, diff, results='hide'}
Ddata.train=data.frame(matrix(NA,nrow=l-deb,ncol=0))
for(i in c("Total","Engie","Cac_40" ,"Taux_Change")) 
{ 
  print(summary(ur.df(diff(data.train[,i],lag=1,differences=1),type="none",lag=1)))
  Ddata.train[,i]=diff(data.train[,i],lag=1,differences=1)
}
colnames(Ddata.train)=c("D.Total","D.Engie","D.Cac_40" ,"D.Taux_Change")
plot.ts(Ddata.train)

head(Ddata.train[,c(1,3)],20)
```
...
```{r, test Johansen}
test=ca.jo(data.train[,c(2,4,5)], type="trace", K=3, ecdet="none", spec="longrun")
summary(test)

```
Il apparaît qu'avec 4 variables il y a une relation de cointégration, en effet r=0 est rejeté, mais r<=1 ne l'est pas, on peut donc conclure r=1.


## Modèle VAR sur les trois variables "Total","Cac_40","Taux_Change"


```{r, test Johansen2}
test=ca.jo(data.train[,c("Total","Cac_40","Taux_Change")], type="trace", K=3, ecdet="none", spec="longrun")
summary(test)

```
Cette fois-ci r=0 n'est pas rejetable. Donc l'absence de cointégration rend licite l'utilisation du modèle VAR sur les différentielles premières.

### recherche d'un décalage adapté.
```{r, recherche_lag2}
colnames(Ddata.train)=colnames(data.train[,2:5])
Def=Ddata.train[,c("Total","Cac_40","Taux_Change")]
lag=lags.select(Def,lag.max =4,include="none")
print(lag)
```
Nous retenons un VAR(1).

## Estimation du modèle, racines unitaires et test sur les résidus
...
par(mfrow=c(3,1))
hist(resid(varModel)[,1],breaks=c(-3,-2,-1,0,1,2,3))
hist(resid(varModel)[,2])
hist(resid(varModel)[,3])
par(mfrow=c(3,1))
acf(resid(varModel)[,1],lag=10)
acf(resid(varModel)[,2],lag=10)
acf(resid(varModel)[,3],lag=10)
par(mfrow=c(3,1))
pacf(resid(varModel)[,1],lag=10)
pacf(resid(varModel)[,2],lag=10)
pacf(resid(varModel)[,3],lag=10)
```
